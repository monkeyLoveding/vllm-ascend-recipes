  ...
  return (
    <main className="py-6 w-full min-w-0">
      ...
      <section className="space-y-2">
        {(guide || guideZh) && (
          <Accordion title="Guide" defaultOpen>
            {guide && (
              <div className="guide-content guide-en">
                <Markdown
            ...
            {guideZh && (
              <div className="guide-content guide-zh hidden">
                <Markdown
                  remarkPlugins={[remarkGfm]}
                  rehypePlugins={[rehypeSlug]}
                >
                ...
                __html: `
                  (function() {
                    function switchGuide() {
                      var lang = 'en';
                      try { lang = localStorage.getItem('vllm-recipes-lang') || 'en'; } catch(e) {}
                      var enEls = document.querySelectorAll('.guide-en');
                      var zhEls = document.querySelectorAll('.guide-zh');
                      for (var i = 0; i < enEls.length; i++) enEls[i].classList.toggle('hidden', lang === 'zh');
                      for (var i = 0; i < zhEls.length; i++) zhEls[i].classList.toggle('hidden', lang !== 'zh');
                    }
                    switchGuide();
                    window.addEventListener('langchange', switchGuide);
                  })();
                `,
              }}
            />
      ...
      <footer className="mt-10 pt-4 border-t border-border text-sm text-muted-foreground flex flex-wrap gap-x-4 gap-y-2">
        ...
        <a
          href={`https://github.com/vllm-project/vllm-ascend-recipes/edit/main/models/${recipe.hf_org}/${recipe.hf_repo}.yaml`}
          className="inline-flex items-center gap-1 hover:text-foreground transition-colors"
        >
          <Pencil size={12} /> Edit recipe
        </a>
        <a
          href="https://github.com/vllm-project/vllm-ascend-recipes/issues"
          className="inline-flex items-center gap-1 hover:text-foreground transition-colors"
        >
