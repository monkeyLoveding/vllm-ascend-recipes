...
export function computeDockerMeta(recipe, variant, hwProfile, hwId = null) {
  ...
  const DEFAULT_IMAGE = nightlyRequired
    ? {
        ...
        ascend: "quay.io/ascend/vllm-ascend:nightly",
      }
    : {
  ...
  const isAscend = hwProfile?.brand === "Ascend";
  ...
  const brandKey = isTpu ? "tpu" : isAmd ? "amd" : isAscend ? "ascend" : isIntel ? "intel" : "nvidia";
  ...
  const gpuFlags = isTpu
    ...
    : isIntel
      ? "--shm-size=16g"
      : "--gpus all";
  return { image, gpuFlags, brandKey, isAmd, isAscend, isTpu, isXpu, isIntel, pinned, cudaMap, nightlyRequired };
}
